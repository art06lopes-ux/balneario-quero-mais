-- ==============================================================
-- CORRIGIR: remove o esquema da Facundes (rodado por engano) e
-- aplica o do Balneário Quero Mais. Cole tudo no SQL Editor e Run.
-- ==============================================================
...
drop policy if exists "storage: leitura publica dos buckets do site" on storage.objects;
drop policy if exists "storage: envio autenticado" on storage.objects;
drop policy if exists "storage: atualizacao autenticada" on storage.objects;
drop policy if exists "storage: remocao autenticada" on storage.objects;
-- (os buckets vazios "services" e "gallery" ficam: o SQL não pode apagá-los e não atrapalham)

-- Caso o esquema do balneário tenha entrado parcialmente, limpa também.

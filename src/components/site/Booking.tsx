  ...
  return (
    ...
    >
      <div className="mx-auto grid w-[min(1180px,100%-2.5rem)] items-center gap-[clamp(2.5rem,6vw,5rem)] md:grid-cols-2">
        <div>
          ...
          <Reveal delay={0.15}>
            <ol className="mt-8 grid gap-3">
              {["Preencha os dados", "Veja o total calculado na hora", "Confirme pelo WhatsApp"].map((s, i) => (
                <li key={s} className="flex items-center gap-3.5 font-medium">
                  <span className="grid size-[34px] shrink-0 place-items-center rounded-full bg-sun-500 font-display font-extrabold text-forest-900">{i + 1}</span>
                  {s}
                </li>
              ))}
            </ol>
        ...
        <Reveal delay={0.1}>
          <div id="ticket-card" className="relative overflow-hidden rounded-xl2 bg-white text-ink shadow-deep before:absolute before:top-[92px] before:-left-3.5 before:z-[2] before:size-7 before:rounded-full before:bg-forest-900 after:absolute after:top-[92px] after:-right-3.5 after:z-[2] after:size-7 after:rounded-full after:bg-forest-900">
            ...
            <form onSubmit={submit} noValidate className="grid gap-4 px-8 pt-7 pb-6 max-sm:px-5">
              <div className="grid gap-1.5">
                <label htmlFor="qty" className="font-display text-sm font-semibold text-ink-2">Quantidade de pessoas</label>
                <div className="grid grid-cols-[52px_1fr_52px] overflow-hidden rounded-xl border-[1.5px] border-[#d8e2dc] bg-[#fafcfb]">
                  <button type="button" onClick={() => setQty((q) => Math.max(1, q - 1))} aria-label="Diminuir" className="bg-forest-100 text-2xl font-bold text-forest-800 transition-colors hover:bg-[#d5eadd]">−</button>
                  <input
                    id="qty"
                    type="number"
                    inputMode="numeric"
                  ...
                  />
                  <button type="button" onClick={() => setQty((q) => Math.min(99, q + 1))} aria-label="Aumentar" className="bg-forest-100 text-2xl font-bold text-forest-800 transition-colors hover:bg-[#d5eadd]">+</button>
                </div>
              </div>
              ...
              <div className="grid gap-1.5">
                <label htmlFor="name" className="font-display text-sm font-semibold text-ink-2">Seu nome</label>
                <input id="name" type="text" autoComplete="name" placeholder="Como devemos te chamar?" value={name} onChange={(e) => setName(e.target.value)} className={inputCls(invalid.name)} />
              </div>

              <div className="grid gap-1.5">
                <label htmlFor="phone" className="font-display text-sm font-semibold text-ink-2">Seu WhatsApp</label>
              ...
              <p className="text-center text-[0.8rem] leading-relaxed text-ink-3">
                A confirmação acontece na conversa pelo WhatsApp. Nenhuma cobrança é feita pelo site.
              </p>
            </form>
          </div>
        </Reveal>

...
export function GeneralForm({ row, urls }: { row: SettingsRow; urls: Urls }) {
  ...
  return (
    <UnsavedGuard savedKey={state.success}>
      <form action={action} className="grid gap-6">
        <Card title="Hero (topo do site)">
          <div className="grid gap-4">
            ...
            <Field
              label="Título"
              htmlFor="hero_title"
              hint="As duas últimas palavras ficam em amarelo."
            >
              <Input
                id="hero_title"
                name="hero_title"
                defaultValue={row.hero_title}
                maxLength={80}
                required
              />
            </Field>
            <Field label="Frase de impacto" htmlFor="hero_subtitle">
              <Textarea
                id="hero_subtitle"
                name="hero_subtitle"
                defaultValue={row.hero_subtitle}
                maxLength={300}
              />
            </Field>
            <ImageField
              ...
              current={urls.hero}
              hint="Use a foto mais impactante, na horizontal. Ela também é a imagem de compartilhamento no WhatsApp/Instagram."
            />
          </div>
        </Card>
        ...
        <Card title="Sobre o balneário">
          <div className="grid gap-4">
            ...
            <ImageField
              name="about_image"
              label="Foto principal (vertical)"
              current={urls.about}
              aspect="aspect-[4/5]"
            />
            <ImageField
              name="about_image_secondary"
              label="Foto secundária (quadrada)"
              current={urls.aboutSecondary}
              removable
        ...
        <Card title="Logo e redes">
          <div className="grid gap-4">
            <ImageField
              ...
              current={urls.logo}
              aspect="aspect-square"
              hint="Arquivo quadrado (PNG ou JPG). Ela aparece no cabeçalho, no hero e no rodapé."
            />
        ...
        <Card title="Prova social">
          <p className="mb-4 text-sm text-ink-3">
            Aparece no topo do site e na chamada final. Deixe vazio para esconder. Atualize de
            tempos em tempos com o que o Google e o Instagram mostram.
          </p>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Nota no Google (0 a 5)" htmlFor="google_rating">
              <Input
                ...
                name="google_rating"
                inputMode="decimal"
                defaultValue={row.google_rating ?? ""}
                placeholder="4,7"
              />
            </Field>
            <Field label="Quantidade de avaliações" htmlFor="google_rating_count">
              <Input
                ...
                inputMode="numeric"
                defaultValue={row.google_rating_count ?? ""}
                placeholder="54"
              />
            </Field>
            <Field
            ...
            >
              <Input
                ...
                type="url"
                defaultValue={row.google_reviews_url ?? ""}
              />
            </Field>
            <Field label="Seguidores no Instagram (texto livre)" htmlFor="instagram_followers">
              <Input
                ...
                defaultValue={row.instagram_followers ?? ""}
                placeholder="7,9 mil"
                maxLength={40}
              />
            </Field>
  ...
  return (
    <UnsavedGuard savedKey={state.success}>
      <form action={action} className="grid gap-6">
        <Card title="WhatsApp">
          <Field
            label="Número que recebe as reservas"
            htmlFor="whatsapp_number"
            hint={`Só números, com DDI e DDD. Hoje: ${formatWhatsappNumber(row.whatsapp_number)}. Ex.: 5592999999999`}
          >
            <Input
              id="whatsapp_number"
              name="whatsapp_number"
              inputMode="numeric"
              defaultValue={row.whatsapp_number}
              pattern="[0-9]{10,15}"
              required
            />
          </Field>
          <p className="mt-3 text-xs text-ink-3">
            Este número é usado em todos os botões de reserva, no botão flutuante e na seção de
            localização.
          </p>
        </Card>
          ...
          <div className="grid gap-4">
            ...
            <Field label="Quando a entrada é cobrada" htmlFor="charge_mode">
              ...
              >
                ...
                <option value="sundays_holidays">
                  Só aos domingos e feriados (demais dias grátis)
                </option>
              </select>
            </Field>
            <Field
            ...
            >
              <Textarea
                id="extra_holidays"
                name="extra_holidays"
                defaultValue={row.extra_holidays ?? ""}
                placeholder={"05/09\n08/12"}
                className="max-w-[360px]"
              />
            </Field>
            <Field
              label="Frase junto do preço (opcional)"
              htmlFor="pricing_note"
              hint="Vazio = texto automático (Cobrada aos domingos e feriados. Nos demais dias, entrada gratuita.)"
            >
              <Input
                id="pricing_note"
                name="pricing_note"
                defaultValue={row.pricing_note ?? ""}
                maxLength={160}
              />
            </Field>
            <p className="text-xs text-ink-3">
              O total da reserva é calculado pela data escolhida: em dia sem cobrança, o site mostra
              &ldquo;Grátis&rdquo;.
            </p>
          </div>
        </Card>
      ...
      <form action={action} className="grid gap-6">
        <Card title="Endereço e funcionamento">
          <div className="grid gap-4">
            ...
            <Field
              ...
              hint="Aparece abaixo das informações. Ex.: como chegar, estacionamento, avisos."
            >
              <Textarea
                id="location_notes"
                name="location_notes"
                defaultValue={row.location_notes}
                maxLength={1000}
              />
            </Field>
          </div>
        ...
        <Card title="Mapa">
          <Field
            ...
            htmlFor="maps_query"
            hint="O que o mapa procura para colocar o pino. O nome do balneário costuma funcionar melhor que o endereço. Deixe vazio para esconder o mapa."
          >
            <Input
              id="maps_query"
              name="maps_query"
              defaultValue={row.maps_query}
              maxLength={200}
            />
          </Field>
        </Card>

    ...
    <div className="grid gap-6">
      ...
      {adding && (
        <Card title="Novo prato">
          <FoodForm onDone={(s) => { setToast(s); if (s.success) setAdding(false); }} />
        </Card>
      )}
      ...
      {items.length === 0 ? (
        ...
        <Sortable items={items} onReorder={reorderFood} onResult={setToast} layout="grid" render={(f) => <FoodCard f={f} onToast={setToast} />} />
      )}
      <Toast state={toast} />
    </div>
  );
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="relative aspect-[4/3] overflow-hidden rounded-t-2xl bg-forest-100">
        ...
        {f.price !== null && (
          <span className="absolute bottom-2 left-2 rounded-full bg-sun-500 px-2.5 py-0.5 font-display text-xs font-extrabold text-forest-900">{formatBRL(f.price)}</span>
        )}
        {!f.is_active && <span className="absolute top-2 left-2 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-semibold text-ink-3 uppercase">oculto</span>}
      </div>
      <div className="flex flex-1 flex-col p-4">
        ...
        {editing && (
          <div className="mt-4 border-t border-forest-900/10 pt-4">
            <FoodForm food={f} onDone={(s) => { onToast(s); if (s.success) setEditing(false); }} />
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <form action={action} className="grid gap-4">
      ...
      <Field label="Descrição (opcional)" htmlFor={`desc-${k}`}>
        <Textarea id={`desc-${k}`} name="description" defaultValue={food?.description} maxLength={400} className="min-h-[80px]" />
      </Field>
      <Field label="Preço (opcional)" htmlFor={`price-${k}`} hint="Deixe vazio para não exibir preço. Ex.: 45,00">
        <Input id={`price-${k}`} name="price" inputMode="decimal" defaultValue={food?.price !== null && food?.price !== undefined ? food.price.toFixed(2).replace(".", ",") : ""} className="max-w-[160px]" />
      </Field>
      ...
      <div className="flex justify-end">
        <Button type="submit" loading={pending}>{food ? "Salvar" : "Adicionar"}</Button>
      </div>
    </form>
  );
}
